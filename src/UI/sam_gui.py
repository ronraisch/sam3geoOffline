...
from .create_overlay_map_leafmap import MAP_BOUNDS
...
# def add_offline_raster(
#     m: MapWrapper, tif_path: str, overlay_bounds: MAP_BOUNDS, opacity: float = 1.0
# ):
#     """
#     Converts a .tif to a static PNG and adds it as an ImageOverlay.
#     This bypasses the need for an online/local tile server.
#     """
#     # 1. Open the TIF and convert to PNG bytes
#     with Image.open(tif_path) as img:
#         # Convert to RGBA if necessary to handle transparency/nodata
#         img = img.convert("RGBA")

#         # Optional: Apply your 'nodata=0' logic here by manipulating the alpha channel
...
#     # 2. Extract bounds (If you have georeferenced TIFs, use rasterio to get bounds)
#     # For now, we'll assume you have the overlay_bounds variable available
#     new_layer = ImageOverlay(
#         url=uri,
#         bounds=overlay_bounds,  # Ensure these are the correct lat/lon bounds
#         name=m.layer_name,
#         opacity=opacity,
#     )
